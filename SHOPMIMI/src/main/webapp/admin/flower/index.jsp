                        ...
                        <div class="table-responsive">
                            <div class="row">
                                ...
                                if("admin".equals(userLogin.getApiUser().getName()) || "supper mod".equals(userLogin.getApiUser().getName())){
                                        %>
                                    <a href="<%=request.getContextPath() %>/admin/flower/add" class="btn btn-success btn-md">Thêm</a>
                               <%} %>
                                </div>
                            ...
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <%	@SuppressWarnings("unchecked")
                                	...
                                	if(flowers != null && flowers.size() > 0){
                                		for(Flower items : flowers){
                                %>
                                    <tr>
                                        <td><%=items.getId() %></td>
                                        <td class="center"><%=items.getName() %></td>
                                        <td class="center"><%=format.format((long)items.getPrice()) %></td>
                                        <td class="center"><%=items.getTypeFlower().getName() %></td>
                                        <td class="center"><%=items.getCounter() %></td>
                                        <td class="center">
											<img width="200px" height="200px" src="<%=request.getContextPath()%>/fileFlowers/<%=items.getPicture() %>" alt="Ảnh"/>
                                        </td>
                                        ...
                                        if("admin".equals(userLogin.getApiUser().getName()) || "supper mod".equals(userLogin.getApiUser().getName())){
                                        %>
                                            <a href="<%=request.getContextPath()%>/admin/flower/edit?id=<%=items.getId() %>" title="" class="btn btn-primary"><i class="fa fa-edit "></i> Sửa</a>
                                            <a href="<%=request.getContextPath()%>/admin/flower/del?id=<%=items.getId() %>" title="" class="btn btn-danger" onclick="return confirm('Bạn có chắc muốn xoá!');"><i class="fa fa-pencil"></i> Xóa</a>
                                        <%} %>
                                        </td>
                                    </tr>
                                    <%}}else{ %>
                                    <tr><td colspan="8" align="center">Chưa có sản phẩm nào</td></tr>
                                    <%} %>
                                </tbody>
                            </table>
                            <%
                                ...
                                <div class="col-sm-6" style="text-align: right;">
                                    <div class="dataTables_paginate paging_simple_numbers" id="dataTables-example_paginate">
                                        ...
                                        %>
                                            ...
                                            <%}else{ %>
                                            <li class="paginate_button previous" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_previous"><a href="<%=request.getContextPath() %>/admin/flowers?page=<%=currentPage-1 %>">Trang trước</a></li>
                                            <%} %>
                                            <%
                                            String active ="";
                                            for(int i= 1; i<= numberOfPages; i++){
                                            	...
                                            	%>
                                            <li class="paginate_button <%=active %>" aria-controls="dataTables-example" tabindex="0"><a href="<%=request.getContextPath() %>/admin/flowers?page=<%=i %>"><%=i %></a></li>
											<%} %>
											<%
                                        ...
                                        <%}else{ %>
                                        <li class="paginate_button previous next" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_previous"><a href="<%=request.getContextPath() %>/admin/flowers?page=<%=currentPage+1 %>">Trang tiếp</a></li>
                                        <%} %>
                                        </ul>
                                    </div>
                                </div>
